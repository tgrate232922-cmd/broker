    ...
    public function joinPlan()
    {
        ...
        if (empty($this->amountToInvest)) {
            ...
            if ($user->account_bal < $plan_price) {
                session()->flash('message', 'Insufficient balance. Please fund your account to continue.');
            } else {
                ...
                session()->flash('success', 'Your staking pool has been successfully activated and is now live.');
                $this->amountToInvest = 0;
                $this->disabled = 'disabled';
