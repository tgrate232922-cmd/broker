        ...
        <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-gray-900 p-3 sm:p-4 shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 flex flex-col">
            ...
            <div class="flex items-start justify-between gap-2 mb-3">
                ...
                <div class="w-8 h-8 flex items-center justify-center rounded-xl bg-gray-100/80 dark:bg-gray-800/80 ring-1 ring-gray-200 dark:ring-gray-700">
                    <i data-lucide="{{ $card['icon'] }}" class="w-4 h-4 text-gray-600 dark:text-gray-300"></i>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex-1 min-w-0 text-right">
                    ...
                    <!-- trend badge -->
                    <div class="mt-1 inline-flex items-center justify-end gap-1 text-[11px] px-2 py-1 rounded-full {{ $trendBadge }}">
 ...
 <!--   @endif-->
  ...
  <div class="xl:col-span-2 rounded-2xl overflow-hidden shadow ring-1 ring-white/10 bg-gradient-to-b from-slate-950/80 to-slate-900/70 backdrop-blur-xl">
    ...
    <div class="relative p-4 sm:p-6">
        ...
        <!-- header -->
       {{-- Top Staking Pools • Live (Glass + Chart + List) --}}
                    ...
                    <div class="flex items-end gap-1 opacity-50">
                        @for($i=0; $i<18; $i++)
                            <div class="w-0.5 rounded bg-white/30 animate-pulse"
                                 style="height: {{ 10 + ($i%6)*6 }}px; animation-delay: {{ $i*60 }}ms;"></div>
                        @endfor
                    </div>
                </div>
...
{{-- Chart.js CDN (no TradingView anymore) --}}
...
async function loadTopPools() {
  ...
  try {
    statusEl.textContent = "Data: Live";
    const res = await fetch("{{ route('dao.pools.live') }}", {
      headers: { "Accept": "application/json" }
    });
...
<script>
  ...
  async function loadPoolsTrend() {
    ...
    try {
      ...
      const res = await fetch("{{ route('dao.pools.live') }}", { headers: { "Accept": "application/json" } });
        ...
        @foreach ($deposits as $deposit)
            ...
            <div class="bg-gray-800 rounded-2xl p-5 border border-gray-700 hover:border-gray-600 transition-colors duration-300">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium {{ $statusBg }}">
            ...
            <div class="bg-gray-800 rounded-2xl p-5 border border-gray-700 hover:border-gray-600 transition-colors duration-300">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="text-white font-semibold text-lg">
                                {{ Auth::user()->currency }}{{ number_format((float)($withdrawal->amount ?? 0), 2) }}
                            </div>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-slate-200 font-medium">
                            {{ Auth::user()->currency }}{{ number_format((float)$deducted, 2) }}
                        </span>
                    </div>
      ...
      <div class="divide-y divide-gray-700">
        @foreach($withdrawals as $withdrawal)
          ...
          <div class="grid grid-cols-12 px-6 py-6 items-center hover:bg-gray-800/50 transition-colors">
            ...
            <div class="col-span-3">
              <div class="text-white font-semibold">
                {{ Auth::user()->currency }}{{ number_format((float)($withdrawal->to_deduct ?? 0), 2) }}
              </div>
                    ...
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <div class="text-right">
                            <div class="text-white font-medium">
                                {{ Auth::user()->currency }}{{ number_format((float)($tx->amount ?? 0), 2) }}
                            </div>
                        </div>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-400">Description</span>
                        <span class="text-slate-300">{{ $tx->plan ?? 'N/A' }}</span>
                    </div>
                    <div class="mt-2 text-xs text-slate-500">
                        {{ \Carbon\Carbon::parse($tx->created_at)->format('M d, Y \a\t g:i A') }}
                    </div>
                </div>
            @endforeach
        @else
                ...
                <tbody class="divide-y divide-gray-700">
                    @foreach ($transactions as $tx)
                        <tr class="hover:bg-gray-800/50 transition-colors duration-300">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-slate-300 font-light">{{ $tx->plan ?? 'N/A' }}</span>
                            </td>
