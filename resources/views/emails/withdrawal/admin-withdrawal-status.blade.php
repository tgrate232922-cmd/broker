...
<head>
  ...
  <title>{{ $subjectLine ?? $headline }} – {{ $brand }}</title>
  <style>
    html,body{
...
<body>
  <div class="preheader">
    Withdrawal update: {{ $amountVal }} {{ $curr }} — {{ $statusLabel }}.
  </div>
        ...
        <table role="presentation" class="container" width="640" cellpadding="0" cellspacing="0" border="0"
          ...
          <tr>
            <td class="px" style="padding:18px 22px;border-bottom:1px solid {{ $stroke2 }};
              ...
              @if(!empty($reason) && $state === 'rejected')
                <div style="margin-top:12px;padding:12px;border:1px solid {{ $stroke2 }};border-radius:14px;background:rgba(11,18,32,.40);">
                  <div style="font-size:12px;color:{{ $muted2 }};font-weight:800;margin-bottom:6px;">Note</div>
                  <div style="font-size:14px;color:{{ $text }};font-weight:600;line-height:1.55;">
                    {{ $reason }}
                  </div>
                </div>
              @endif
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                ...
                <tr>
                  <td style="padding:16px;text-align:center;">
                    <div style="color:{{ $muted2 }};font-size:12px;font-weight:800;letter-spacing:.2px;">
                      AMOUNT
                    </div>
                    <div style="margin-top:8px;color:{{ $text }};font-size:38px;font-weight:900;letter-spacing:.2px;">
                      {{ $amountVal }} <span style="font-size:14px;color:{{ $muted2 }};font-weight:800;">{{ $curr }}</span>
                    </div>
          ...
          <tr>
            <td class="px" style="padding:10px 22px 18px 22px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                ...
                <tr>
                  ...
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $text }};font-weight:900;font-size:13px;">
                    ...
                    <span class="mono" style="color:{{ $muted2 }};margin-left:8px;">{{ $ref }}</span>
                  </td>
                </tr>

                <tr>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $muted2 }};font-size:12px;font-weight:800;">Date &amp; time</td>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $text }};font-weight:900;font-size:13px;">
                ...
                <tr>
                  <td style="padding:14px 14px;color:{{ $muted2 }};font-size:12px;font-weight:800;">Status</td>
              ...
              <div style="color:{{ $muted2 }};font-size:12px;line-height:1.6;text-align:center;">
                This message was sent because you have an account with {{ $brand }}.
              </div>
            </td>
          </tr>
