...
<head>
  ...
  <title>{{ $subjectLine ?? 'KYC Update' }} – {{ $brand }}</title>
  <style>
        ...
        <table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;background:{{ $card }};border:1px solid {{ $border }};border-radius:14px;overflow:hidden;box-shadow:0 14px 38px rgba(3,10,26,.14);">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="color:#e6eef7;font-weight:800;letter-spacing:.2px;font-size:16px;line-height:1.2;">
                    @if(!empty($logo))
                      <img src="{{ $logo }}" alt="{{ $brand }} logo" width="28" height="28" style="vertical-align:middle;border:0;border-radius:6px;margin-right:10px;">
                    @endif
          ...
          <tr>
            <td style="padding:22px;">
              <h1 style="margin:0 0 8px;font-size:20px;line-height:1.35;color:{{ $textColor }};">Hi {{ $fullName }},</h1>
              <p style="margin:8px 0 0;line-height:1.6;font-size:14px;color:{{ $textColor }};">{{ $intro }}</p>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid {{ $border }};border-radius:12px;overflow:hidden;">
                @if($ref)
                  <tr>
                    ...
                    <td style="padding:12px 14px;border-bottom:1px solid {{ $border }};color:{{ $textColor }};font-weight:900;font-size:14px;">{{ $ref }}</td>
                  </tr>
                @endif
                <tr>
                  <td style="padding:12px 14px;border-bottom:1px solid {{ $border }};width:40%;color:{{ $muted }};font-size:12px;font-weight:800;">Date &amp; Time</td>
                  <td style="padding:12px 14px;border-bottom:1px solid {{ $border }};color:{{ $textColor }};font-weight:900;font-size:14px;">
                    {{ $posted_at ? $posted_at->format('Y-m-d H:i') : '' }}
                  </td>
                </tr>
                <tr>
                  <td style="padding:12px 14px;border-bottom:1px solid {{ $border }};color:{{ $muted }};font-size:12px;font-weight:800;">Status</td>
                  <td style="padding:12px 14px;border-bottom:1px solid {{ $border }};color:{{ $textColor }};font-weight:900;font-size:14px;">
                    <span style="display:inline-block;font-size:12px;font-weight:800;padding:6px 10px;border-radius:999px;background:{{ $chipBg }};color:{{ $chipText }};border:1px solid {{ $chipBorder }};">
          ...
          @if(!empty($adminMessage))
            ...
            <tr>
              <td style="padding:12px 22px 8px 22px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid {{ $border }};border-radius:12px;background:#f8fafc;">
                  <tr>
                    <td style="padding:12px 14px;">
                      <div style="color:{{ $muted }};font-size:12px;font-weight:800;margin-bottom:6px;">Message from our team</div>
                      <div style="color:{{ $textColor }};font-size:14px;font-weight:500;line-height:1.6;">
                        {{ $adminMessage }}
                      </div>
                    </td>
                  </tr>
