...
<body>
  <div class="preheader">Your {{ $asset }} {{ $type }} order is now active.</div>
                    ...
                    <table role="presentation" width="100%">
                      <tr>
                        <td style="font-size:18px;font-weight:900;color:#0b1320;">
                          {{ strtoupper($type) }} • {{ $asset }} @if(!empty($symbol)) ({{ $symbol }}) @endif
                        </td>
          ...
          <tr>
            <td style="padding:18px 22px 8px 22px;">
              <table role="presentation" width="100%" style="border:1px solid {{ $border }};border-radius:12px;overflow:hidden;">
                ...
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Opened</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $posted_at->format('Y-m-d H:i') }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Expires</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">@if(!empty($expire_at)) {{ \Carbon\Carbon::parse($expire_at)->format('Y-m-d H:i') }} @else — @endif</td></tr>
