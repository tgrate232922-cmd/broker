{{-- resources/views/emails/bot/investment-created.blade.php --}}
@php
...
<head>
  ...
  <title>Bot Investment Confirmed | {{ $brand }}</title>
  <style>
        ...
        <tr>
          <td style="padding:22px;">
            ...
            <p style="margin:8px 0 0;line-height:1.6;font-size:14px;">
              Your investment in <strong>{{ $bot->name }}</strong> is confirmed.
            </p>
          </td>
        </tr>
            ...
            <table role="presentation" width="100%" style="border:1px solid {{ $border }};border-radius:12px;overflow:hidden;">
              ...
              <tr>
                ...
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">
                  {{ $investment->auto_reinvest ? 'Enabled' : 'Disabled' }}
                  @if($investment->auto_reinvest) ({{ (float)($investment->reinvest_percentage ?? 0) }}%) @endif
                </td>
              </tr>
              <tr>
                <td style="padding:12px 14px;color:#475569;font-size:12px;font-weight:800;">Duration</td>
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">{{ $duration }} days</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">{{ \Carbon\Carbon::parse($investment->expires_at)->format('Y-m-d H:i') }}</td>
              </tr>
            </table>
          </td>
        </tr>
