{{-- resources/views/emails/bot/investment-cancelled.blade.php --}}
@php
  ...
  $amt = number_format((float)($refundAmount ?? 0), 2);
@endphp
<!DOCTYPE html>
...
<head>
  ...
  <title>Bot Investment Cancelled | {{ $brand }}</title>
  <style>
...
<body>
  <div class="preheader">Your bot investment has been cancelled and refunded.</div>
        ...
        <tr>
          <td style="background:{{ $navy }};border-bottom:1px solid rgba(255,255,255,.12);padding:18px 22px;">
            <table width="100%"><tr>
              ...
              <td align="right">
                <span style="display:inline-block;font-size:12px;font-weight:800;padding:6px 10px;border-radius:999px;border:1px solid rgba(255,255,255,.22);background:#0b1220;color:#f4b6bd;">Investment Cancelled</span>
              </td>
            </tr></table>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:22px;">
            ...
            <p style="margin:8px 0 0;line-height:1.6;font-size:14px;">
              Your investment in <strong>{{ $investment->bot->name }}</strong> has been cancelled.
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:0 22px;">
            <table role="presentation" width="100%" style="border:1px solid {{ $border }};border-radius:12px;overflow:hidden;">
              <tr>
                <td style="padding:12px 14px;width:40%;color:#475569;font-size:12px;font-weight:800;">Refund Amount</td>
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">{{ $amt }} {{ $currency }}</td>
              </tr>
              <tr>
                <td style="padding:12px 14px;color:#475569;font-size:12px;font-weight:800;">Started</td>
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">{{ \Carbon\Carbon::parse($investment->started_at)->format('Y-m-d H:i') }}</td>
              </tr>
              <tr>
                <td style="padding:12px 14px;color:#475569;font-size:12px;font-weight:800;">Cancelled</td>
                <td style="padding:12px 14px;color:#0b1320;font-weight:900;font-size:14px;">{{ now()->format('Y-m-d H:i') }}</td>
              </tr>
            </table>
          </td>
        </tr>
