        ...
        <table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;background:{{ $card }};border:1px solid {{ $border }};border-radius:14px;overflow:hidden;box-shadow:0 14px 38px rgba(3,10,26,.14);">
          ...
          <tr>
            <td style="padding:18px 22px 8px 22px;">
              <table role="presentation" width="100%" style="border:1px solid {{ $border }};border-radius:12px;overflow:hidden;">
                <tr><td style="padding:12px 14px;width:40%;color:{{ $muted }};font-size:12px;font-weight:800;">Applicant</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $fullName }} ({{ $user->email }})</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Amount</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $amt }} {{ $currency }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Income</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $income }} {{ $currency }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Purpose</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $purpose }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Facility</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $facility }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Duration</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ $duration }}</td></tr>
                <tr><td style="padding:12px 14px;color:{{ $muted }};font-size:12px;font-weight:800;">Status</td>
                    <td style="padding:12px 14px;color:{{ $text }};font-weight:900;font-size:14px;">{{ ucfirst($status) }}</td></tr>
              </table>
            </td>
          </tr>
