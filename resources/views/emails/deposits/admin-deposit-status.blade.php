...
<body>
  <div class="preheader">
    {{ $headline }} — {{ $amount }} {{ $currencyC }} ({{ strtoupper($method) }}).
  </div>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                ...
                <tr>
                  <td style="padding:16px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td style="font-size:30px;font-weight:950;color:{{ $text }};letter-spacing:.2px;">
                          {{ $amount }}
                          <span style="font-size:14px;font-weight:900;color:{{ $muted2 }};">{{ $currencyC }}</span>
                        </td>
                        <td align="right">
                          <span class="mono" style="font-size:11px;font-weight:950;padding:7px 10px;border-radius:999px;
                                       background:rgba(11,18,32,.55);border:1px solid {{ $stroke2 }};color:{{ $muted }};">
                            {{ strtoupper($method) }}
                          </span>
                        </td>
                      </tr>
          ...
          <tr>
            <td class="px" style="padding:0 22px 18px 22px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                ...
                <tr>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};width:42%;color:{{ $muted2 }};font-size:12px;font-weight:900;letter-spacing:.25px;">
                    REFERENCE
                  </td>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $text }};font-weight:950;font-size:13px;">
                ...
                <tr>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $muted2 }};font-size:12px;font-weight:900;letter-spacing:.25px;">
                    DATE &amp; TIME
                  </td>
                  <td style="padding:14px 14px;border-bottom:1px solid {{ $stroke2 }};color:{{ $text }};font-weight:950;font-size:13px;">
                ...
                <tr>
                  <td style="padding:14px 14px;color:{{ $muted2 }};font-size:12px;font-weight:900;letter-spacing:.25px;">
                    STATUS
                  </td>
                  <td style="padding:14px 14px;color:{{ $text }};font-weight:950;font-size:13px;">
                    {{ ucfirst($statusLower ?: $state) }}
                  </td>
                </tr>
              </table>
