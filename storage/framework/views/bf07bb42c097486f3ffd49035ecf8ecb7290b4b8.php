...
<div class="container mx-auto px-4 py-8" x-data="tradingMarkets()" x-init="init()" x-cloak>
...
<?php endif; ?>
    ...
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Markets</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">
                    Browse instruments and run your vault strategy once per asset.
                </p>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="hidden md:flex items-center gap-4 text-sm">
                    ...
                    <div class="text-center">
                        <div class="text-blue-600 dark:text-blue-400 font-semibold">Live</div>
...
<button @click="selectedType = 'dao_contract'"
 :class="selectedType === 'dao_contract' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
 class="px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2">
  <i data-lucide="file-code-2" class="w-4 h-4"></i>
  DAO Contracts
</button>

<button @click="selectedType = 'top_pools'"
 :class="selectedType === 'top_pools' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
 class="px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2">
  <i data-lucide="crown" class="w-4 h-4"></i>
  Top Pools
</button>

<button @click="selectedType = 'new_pools'"
 :class="selectedType === 'new_pools' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
 class="px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2">
                                        ...
                                        <div class="inline-flex items-center gap-2">
                                            ...
                                            <a :href="`<?php echo e(route('trade.single', '')); ?>/${instrument.id}`"
                                               class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors duration-200 shadow-sm hover:shadow-md">
                                                <i data-lucide="sparkles" class="w-4 h-4"></i>
                                                <span class="hidden sm:inline">Start AI Stake</span>
                                                <span class="sm:hidden">Start</span>
                                            </a>

                                            <!-- View Vault Strategy (secondary) -->
                                            
                                            <a :href="`<?php echo e(route('vault.strategy', '')); ?>/${instrument.id}`"
                                               class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-100 rounded-lg font-medium transition-colors duration-200">
                                                <i data-lucide="vault" class="w-4 h-4"></i>
                                                <span class="hidden sm:inline">View Vault Strategy</span>
    ...
    return {
        ...
        get groupedInstruments() {
            ...
            this.filteredInstruments.forEach(instrument => {
                const t = instrument.type || 'other';
                if (!grouped[t]) grouped[t] = [];
                grouped[t].push(instrument);
            });
...
<?php echo $__env->make('layouts.dasht', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/homefor1/scalpchain.com/account/resources/views/user/trade/trade.blade.php ENDPATH**/ ?>